	...
	if player_in_room && !dying && !spawning:
		...
		if player && can_move && Engine.time_scale != 0.0:
			...
			if can_attack && position.distance_to(player_position) < 100:
				...
				if current_direction == look_direction.left:
					if animated_sprite.animation == "hit_right" || animated_sprite.animation == "hit_idle_right" || animated_sprite.animation == "attack_hit_left" ||animated_sprite.animation == "attack_hit_right" ||animated_sprite.animation == "hit_idle_left":
						var frame = animated_sprite.frame
					...
					if animated_sprite.animation == "hit_left" || animated_sprite.animation == "hit_idle_left" || animated_sprite.animation == "attack_hit_left" ||animated_sprite.animation == "attack_hit_right" ||animated_sprite.animation == "hit_idle_left":
						var frame = animated_sprite.frame
